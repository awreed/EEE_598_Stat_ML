...
def copy_paste(obj_path, background_path, DATASET_PATH):
    obj = cv2.imread(obj_path, 0)
    ...
    for i in range(0, H):
        for j in range(0, W):
            if(obj[i, j] > thresh):
                background[i, j] = obj[i, j]
    
    ret1, binary_image = cv2.threshold(obj, 10, 255, cv2.THRESH_BINARY)
    binary_image, contours, hierarchy = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    assert len(contours) == 1, "Should only be one contour in obj image"
    for c in contours:
        x, y, w, h = cv2.boundingRect(c)
       
    cv2.imwrite(DATASET_PATH + 'images/image.png', background) 
    return x, y, w, h 
